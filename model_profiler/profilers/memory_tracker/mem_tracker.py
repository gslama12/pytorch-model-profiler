# from https://github.com/pytorch/pytorch/blob/main/torch/distributed/_tools/mem_tracker.py
...
class MemTracker(TorchDispatchMode):
    ...
    def __init__(self, record_category_max=False) -> None:
        ...
        self._record_category_max = record_category_max
        # self._debug_history = []
    ...
    def _update_peak_stats(self, peak_state: _State) -> None:
        ...
        for dev, dev_snap in curr_snap.items():
            if self._record_category_max:
                self._peak_mem_snap[dev][_MemRefType.PARAM] = deepcopy(dev_snap[_MemRefType.PARAM]) \
                    if dev_snap[_MemRefType.PARAM] > self._peak_mem_snap[dev][_MemRefType.PARAM] \
                    else self._peak_mem_snap[dev][_MemRefType.PARAM]
                self._peak_mem_snap[dev][_MemRefType.BUFFER] = deepcopy(dev_snap[_MemRefType.BUFFER]) \
                    if dev_snap[_MemRefType.BUFFER] > self._peak_mem_snap[dev][_MemRefType.BUFFER] \
                    else self._peak_mem_snap[dev][_MemRefType.BUFFER]
                self._peak_mem_snap[dev][_MemRefType.ACT] = deepcopy(dev_snap[_MemRefType.ACT]) \
                    if dev_snap[_MemRefType.ACT] > self._peak_mem_snap[dev][_MemRefType.ACT] \
                    else self._peak_mem_snap[dev][_MemRefType.ACT]
                self._peak_mem_snap[dev][_MemRefType.GRAD] = deepcopy(dev_snap[_MemRefType.GRAD]) \
                    if dev_snap[_MemRefType.GRAD] > self._peak_mem_snap[dev][_MemRefType.GRAD] \
                    else self._peak_mem_snap[dev][_MemRefType.GRAD]
                self._peak_mem_snap[dev][_MemRefType.TEMP] = deepcopy(dev_snap[_MemRefType.TEMP]) \
                    if dev_snap[_MemRefType.TEMP] > self._peak_mem_snap[dev][_MemRefType.TEMP] \
                    else self._peak_mem_snap[dev][_MemRefType.TEMP]
                self._peak_mem_snap[dev][_MemRefType.OPT] = deepcopy(dev_snap[_MemRefType.OPT]) \
                    if dev_snap[_MemRefType.OPT] > self._peak_mem_snap[dev][_MemRefType.OPT] \
                    else self._peak_mem_snap[dev][_MemRefType.OPT]
                self._peak_mem_snap[dev][_TOTAL_KEY] = sum(value for key, value in self._peak_mem_snap[dev].items()
                                                           if key != _TOTAL_KEY)
    ...
    def __torch_dispatch__(self, func, types, args=(), kwargs=None):  # type: ignore[no-untyped-def]
        ...
        # self._debug_history.append(self._peak_mem_snap[torch.device("cpu")][_MemRefType.GRAD])
        return res